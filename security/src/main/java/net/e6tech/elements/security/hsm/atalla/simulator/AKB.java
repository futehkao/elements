...
public class AKB {
    ...
    public AKB(String keyBlock) throws GeneralSecurityException {
    ...
    public byte[] decryptKey(byte[] keyEncryptionKey) throws GeneralSecurityException {
        ...
        SecretKey mkey = new SecretKeySpec(xorMKey, "DESede");
        ...
        if (!mac.equals(getMac())) throw new GeneralSecurityException("Mac not verified");
        if (checkDigit == null) checkDigit = calculateCheckDigits(key);
        return key;
    }

    private String generateAKB(String header, byte[] keyEncryptionKey, byte[] key) throws GeneralSecurityException {
        if (key.length % 8 != 0) throw new GeneralSecurityException("key size must be multiple of 8");
        byte[] paddedKey = new byte[24];
        if (key.length == 8) { // single des
            byte padding = Hex.toBytes("53")[0];
            System.arraycopy(key, 0, paddedKey, 0, key.length);
            for (int i = 8; i < 24; i++) paddedKey[i] = padding;
        } else if (key.length == 16) { // double length
            byte padding = Hex.toBytes("44")[0];
            System.arraycopy(key, 0, paddedKey, 0, key.length);
            for (int i = 16; i < 24; i++) paddedKey[i] = padding;
        } else if (key.length == 24) { // triple length
            // do nothing
        } else {
        ...
        String keyBlock = Hex.toString(keyBytes);

        // calculating Mac
        xorMKey = maskAKBEncryptionKey(keyEncryptionKey, Hex.toBytes("4D")[0]);
        String mac = cbcMac(xorMKey, Hex.toBytes(Hex.toString(headerBytes) + keyBlock));

        return header + "," + keyBlock + "," + mac;
    }
    ...
    private String cbcMac(byte[] macKeyBytes, byte[] text) throws GeneralSecurityException {
        ...
        Key macKey = new SecretKeySpec(macKeyBytes, "DESede");
        Cipher cipher = Cipher.getInstance("DESede/CBC/NoPadding");

        if (text.length % 8 != 0) throw new GeneralSecurityException("data block size must be multiple of 8");
    ...
    private String calculateCheckDigits(byte[] key) throws GeneralSecurityException {
        ...
        chkCipher.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(normalizeKey(key), "DESede"));
